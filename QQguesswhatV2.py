...
class DrawPic:
    def __init__(self, image, resizedH=200, step=5):
        self.image = image
    ...
    def draw(self, game=False):
        ...
        for stroke in self.route:
            ...
            if key == 27:
                return 0
            pg.mouseDown(stroke[0][0] * self.zoom + self.upLeft[0], stroke[0][1] * self.zoom + self.upLeft[1],
                         button='left')
            for step in stroke:
                pg.moveTo(step[0] * self.zoom + self.upLeft[0], step[1] * self.zoom + self.upLeft[1])
...
if __name__ == '__main__':
    ...
    option = pg.confirm(text=warning, title='文件缺失', buttons=['画板中绘制', "游戏中绘制", '退出'])
    if option == "游戏中绘制":
        game = True
    else:
        game = False
